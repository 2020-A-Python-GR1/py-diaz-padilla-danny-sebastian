...
class MainView:
    ...
    def __init__(self, root):
        ...
        # ROW 0
        Label(self.frame, text=s.dictionary['text_create_grupal_chat']).grid(row=0, column=0, columnspan=2)
        Button(self.frame, text=s.dictionary['string_create'], command=lambda: self.nav_to_create_grupal_chat(ChatGroup.generateNewId())).grid(row=0, column=2)


        # ROW 1
        self.entry_search = StringVar()
        Entry(self.frame, textvariable=self.entry_search).grid(row=1, column=0)
        Button(self.frame, text=s.dictionary['string_search'], command=lambda: self.search_group(self.entry_search.get())).grid(row=1, column=1)
        Button(self.frame, text=s.dictionary['text_manage_integrants'], command=lambda: self.nav_to_manage_integrants()).grid(row=1, column=2)
    ...
    def nav_to_update_group(self, selection):
        if self.is_valid_selection(selection):
            UpdateChatGroupView(Toplevel(self.root), self.listbox.get(selection[0]))
        else:
